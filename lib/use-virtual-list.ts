...
function useVirtualListResources<T>(list: MaybeRef<readonly T[]>, containerRef?: UseVirtualListContainerRef): UseVirtualListResources<T> {
  containerRef = containerRef ?? shallowRef<HTMLElement | null>(null);
...
function useHorizontalVirtualList<T>(options: UseHorizontalVirtualListOptions, list: MaybeRef<readonly T[]>) {
  const resources = useVirtualListResources(list, options.containerRef)
